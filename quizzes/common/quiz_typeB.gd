...
func calculate_won_energy() -> int:
	...
	for d in _draggables:
		...
		var myDZone : DropZone = d.get_parent()
		if myDZone == d.correct_dropzone: energy_won = d.energy_if_correct
		elif myDZone.is_storage: energy_won = d.energy_if_in_stogage
		else: energy_won = d.energy_if_uncorrect
		var s := ""; var c: Color
